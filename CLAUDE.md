...
### Code editor (Monaco), browser only

**Rule: every code-editing feature runs in the browser.** Autocomplete, formatting and highlighting never call the server. The backend is for running code, problems, users and discussions only.

- **Editor:** the sandbox and problem pages use Monaco. It's vendored under `static/vendor/monaco-editor-*/` and regenerated by `scripts/vendor-monaco.sh`, which prunes unused language workers.
  - `static/js/editor/java-editor.js` (`createJavaEditor`) is the only place Monaco is configured: rainbow brackets, guides, IntelliJ keys (Ctrl+Alt+L format, Ctrl+Enter run, Ctrl+D duplicate), and the status bar.
...
- **Autocomplete:** `static/js/editor/java-completions.js` (pure functions).
- **Formatting (Ctrl+Alt+L):** `static/js/editor/java-formatter.js`, using Prettier with prettier-plugin-java.
  - Vendored by `scripts/vendor-prettier-java.sh`, pinned to plugin 2.8.1, the last version with a pure-JS parser. Newer versions need WebAssembly, which would mean adding `'wasm-unsafe-eval'` to the CSP.
  - Method bodies are formatted inside a wrapper class, and error line numbers are corrected for the wrapper.
- **Tests:** both modules are unit-tested with `node --test`.
